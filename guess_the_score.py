...
def get_game_thread(reddit):
	...
	for submission in user.submissions.new(limit=50):
		if submission.subreddit == 'canucks' and submission.title.startswith('Game Thread:'):
...
def get_guesses(reddit, OUR_TEAM_ID, LAST_GAME_OPPONENT_ID, last_game_start_time, username):
	...
	for reply in comment.replies:
		...
		if reply.delete == True:
			print("deleted account, moving on", reply.body)
			continue

		# Skip removed comments
		if reply.banned_by != None:
			print("deleted or removed comment, moving on", reply.body)
			continue
		...
		try:
			...
			print("Incorrect Format - ", reply.id, reply.body)
			continue
...
def update_scoreboard(reddit, guesses, game_score):
	...
	reddit.subreddit('canucks').wiki['gtsleaderboard'].edit(content=scoreboard, reason="scoreboard_update")
...
***
[View Scoreboard](https://www.reddit.com/r/canucks/wiki/gtsleaderboard/)  
&nbsp;  
